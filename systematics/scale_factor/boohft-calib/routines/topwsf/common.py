...
def resolve_topwsf_fileset(cfg):
    ...
    for era in eras:
        ...
        missing_data = data_samples - {sample for found_era, sample in found_nominal_samples if found_era == era}
        if missing_samples or missing_data:
            missing_bits = []
            if missing_samples:
                missing_bits.append("MC: " + ", ".join(sorted(missing_samples)))
            if missing_data:
                missing_bits.append("data: " + ", ".join(sorted(missing_data)))
            raise FileNotFoundError(f"Missing topwsf nominal input file(s) for {era}: " + "; ".join(missing_bits))
