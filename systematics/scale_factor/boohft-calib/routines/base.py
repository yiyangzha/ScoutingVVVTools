...
def _validate_required_branches(tree, branches, path):
    if branches is None:
        return None
    available = set(tree.keys())
    missing = sorted(branch for branch in branches if branch not in available)
    if missing:
        raise KeyError(f"Missing required Events branch(es) in {path}: {', '.join(missing)}")
    return branches
...
def _run_dataset_fallback(arg):
    ...
    for fname in files:
        with uproot.open(fname + ':Events', handler=MemmapSource) as tree:
            dataset_branches = _validate_required_branches(tree, branches, fname)
    ...
    def run_coffea_job(self):
        ...
        for dataset, files in self.fileset.items():
            ...
            if branches is not None:
                for fname in files:
                    with uproot.open(fname + ':Events', handler=MemmapSource) as tree:
                        _validate_required_branches(tree, branches, fname)
