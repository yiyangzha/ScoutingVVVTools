...
def calibration_selection(calibration):
    selection = str(calibration["selection"])
    jetht_trigger = calibration.get("jetht_trigger", {})
    if jetht_trigger is None:
        return selection
    if not isinstance(jetht_trigger, dict):
        raise SystemExit("calibration.jetht_trigger must be an object")
    if not jetht_trigger.get("enabled", False):
        return selection
    branch = str(jetht_trigger.get("ntuple_branch", "passJetHT"))
    if not branch.isidentifier():
        raise SystemExit(f"calibration.jetht_trigger.ntuple_branch is not a valid ntuple branch name: {branch}")
    return f"({selection}) & ({branch})"
...
def generated_card(cfg, samples, data_names, mc_groups, year, target, tagger_name, tagger_cfg):
    ...
    payload = {
        ...
        "selection": calibration_selection(cal),
