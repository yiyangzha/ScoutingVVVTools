...
Generated Condor workers and submit wrappers source CVMFS setup scripts with `set -u` temporarily disabled and then restore the previous shell flags, because CERN LCG/module setup scripts are not guaranteed to be nounset-safe.
...
This creates the requested Condor work directory, copies a merged config snapshot, packs the repository, packages a minimal `worker_runtime.tar.gz` from the local build, writes `submit.jdl`, `submit_lxplus.jdl`, and `submit.sh`. For IHEP CMS jobs, the generated JDL sets `AcctGroup="cms"` and `HepJob_WallTime="mid"` directly.
Mode 11 can run its controller with ordinary Python when the fixed CVMFS LCG view is available; otherwise it falls back to the pixi/conda environment. The C++ build and Condor workers use `/cvmfs/sft.cern.ch/lcg/views/LCG_109/x86_64-el9-gcc13-opt` directly when it exists, with no local LCG environment variable needed. In LCG mode, ROOT, correctionlib, yaml-cpp, and Python3 CMake inputs are resolved from the sourced LCG setup environment, including its `CMAKE_PREFIX_PATH` and Python package paths; yaml-cpp is linked through an explicit imported target using the resolved LCG include directory and library path, and each executable links that target directly so the build cannot fall back to a system yaml-cpp library. The worker sources that setup script and the runtime bundle contains only `nano_run` plus local build products. In fallback mode, the bundle also carries the private pixi/conda shared-library closure needed by `nano_run`. `hadd`, `xrdcp`, and `xrdfs` are always resolved from the worker node or fixed CVMFS LCG environment, not copied from pixi.
The runtime bundle requires `nano_run` and local shared libraries that are loaded at runtime; `libnanoaodtools_cpp.so` is included only when a shared build produced it, because the default `nanoaodtools_cpp` target is linked into `nano_run` as a static library.

Before submitting regenerated mode-11 jobs on lxplus, run the preflight script from the repository root:

```bash
bash systematics/scale_factor/check_lxplus_mode11.sh
```
...
Pass the base local output directory, not the `pieces/` subdirectory. `nano_merge` reads input pieces from `<output-dir>/pieces/` by default. For Tier pieces, keep the first argument local and pass the remote pieces directory explicitly:

```bash
build/nano_merge /path/to/local/output --pieces-dir root://cceos.ihep.ac.cn//eos/ihep/cms/store/user/yiyangz/Research/VVV/ScoutingVVVTools_sf/scale_factor/ntuples/.../pieces
```
