...
- `mode=4`: run `plotting/data_mc.py`.
  ...
  Output: one PDF per plotted branch under the configured `output_root`, plus `plotting/log.txt`. When the copied BDT config reads MC from `{sample_group}_mixed`, data loading falls back to the original `{sample_group}` path so data can remain under `dataset/data/`.
