...
Mode 11 resolves DAS datasets before submission. The controller prefers `/cvmfs/cms.cern.ch/common/dasgoclient` when available; set `SCALE_FACTOR_DASGOCLIENT=/path/to/dasgoclient` only to override it explicitly. DAS queries use quoted DAS QL values first, keep the legacy unquoted forms as fallbacks, and run with pixi/conda variables removed, active `CONDA_PREFIX` paths stripped from path-like variables, and `/cvmfs/cms.cern.ch/cmsset_default.sh` sourced immediately before executing the CVMFS DAS client.
