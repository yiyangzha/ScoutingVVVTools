...
- `mode=2`: run `selections/BDT/train.py`.
  ...
  Output: one trained BDT directory per tree under `output_root`, containing the model, plots (one `roc_{sig_name}.pdf` per signal class with all background classes overlaid in distinct colors, per-signal-vs-background `score_*` plots, `importance.pdf`, the three `loss_*.pdf` loss curves with y-axis starting at 0, `feature_corr.pdf`, and `decor_corr_*.pdf`), copied `config.json` / `branch.json` / `selection.json`, `test_ranges.json`, and the saved test-set prediction references `test_reference_signal_region.npz` / `test_reference_qcd_est.npz`, plus `selections/BDT/log.txt`. During training, `train.py` logs six loss values per boosting round: train/test `classification_loss`, `decorrelation_loss`, and `total_loss`. For the custom-objective decorrelation modes (`smooth_cvm` and `cvm`), the classification term uses native weighted-sum XGBoost softprob (`Σ w·(-log p_true)`) with native per-row softprob grad/Hess, the tree regularization stays in native XGBoost `Ω(f)` handling (`gamma`, `reg_lambda`, `reg_alpha`), and the logged `total_loss` adds that native regularization back on top of the recorded classification and decorrelation losses. The decorrelation term is multiplied by one fixed train-split scale calibrated once from a classification-only pilot of `max(20, n_estimators // 4)` rounds so that `decor_lambda=1` keeps the logged `decor_loss` at a magnitude comparable to `cls_loss + reg_loss` at the pilot point; the per-event decor Hessian is then floored at `c·|grad|` (with `c` taken from the pilot as `median(hess_cls) / median(|grad_cls|)` when the top-level `decor_hess_floor_c` is `"pilot"`, or from the configured number otherwise) to prevent overshoot when the Gauss-Newton surrogate underestimates curvature. Early stopping is based on test `total_loss`, with the per-tree `early_stopping_rounds` hyperparameter (default 10). The saved reference files keep the pre-lumi physics test weights, and downstream comparison multiplies them by the configured `lumi` before checking the reconstructed physical weights.
...
- `selections/BDT/config.json`: BDT inputs, class groups, training settings, output directories, top-level `decor_loss_mode`, `decor_lambda`, and `decor_hess_floor_c` (`"pilot"` or a non-negative number), and per-tree hyperparameters (`n_estimators`, `max_depth`, `learning_rate`, `gamma`, `reg_lambda`, `reg_alpha`, `min_child_weight`, `early_stopping_rounds`), `decorrelate`, and `event_reweight_branches` (per-event raw-weight branches multiplied before sample normalisation; default `["weight_pu"]`).
