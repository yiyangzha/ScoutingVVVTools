...
Submit jobs either by setting `ntuple.submit_condor: true` or manually from each generated job directory:
...
- `calibration.template_pt_bins`: the shared template pT axis; the Scouting SF card starts at 170 GeV with 10 GeV bins.
- `calibration.systematics.enabled`: currently `pu`, `jms`, `jmr`, `lhescalemuf`, and `lhescalemur`. JES/JER/MET remain listed but disabled because the Scouting inputs do not provide the required shifted reconstruction inputs.
...
Missing configured samples, missing xsections/lumi, missing configured ScoutingNano branches, missing ntuple branches, missing local ntuple files, and missing `Runs/genEventSumw` fail explicitly. Scouting MC requires the complete configured `GenPart_*` ancestry so the ntuple maker can reproduce the original offline W/top matching; data jobs remove these MC-only branches before binding. Every processed input file contributes its complete `Runs` entries, including files with no selected events, so `genEventSumw` and LHE-scale normalization remain inclusive. NanoAOD `LHEScaleSumw` ratios are combined after weighting each Runs entry by its corresponding `genEventSumw`.
