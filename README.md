...
- `mode=2`: run `selections/BDT/train.py`.
  ...
  Output: one trained BDT directory per tree under `output_root`, containing the main model `{tree}_model.json`, the stage-1 baseline `{tree}_model_stage1.json`, shared `feature_corr.pdf` and `loss_*.pdf`, stage-tagged comparison plots such as `importance_cls.pdf` / `importance_decorr.pdf`, `roc_*_cls.pdf` / `roc_*_decorr.pdf`, `score_*_cls.pdf` / `score_*_decorr.pdf`, and `decor_corr_{train,test}_cls.pdf` / `decor_corr_{train,test}_decorr.pdf`, plus copied `config.json` / `branch.json` / `selection.json`, `test_ranges.json`, and the saved test-set prediction references `test_reference_signal_region.npz` / `test_reference_qcd_est.npz`, plus `selections/BDT/log.txt`. When decorrelation is enabled, `train.py` first runs a native `multi:softprob` stage with `n_estimators`, `learning_rate`, and `early_stopping_rounds`, then resumes from the stage-1 best model for `n_estimators_decorr` additional rounds at `learning_rate_decorr` using the custom decorrelation objective. Stage 1 logs only train/test `total_loss`; stage 2 logs train/test `classification_loss`, `decorrelation_loss`, and `total_loss`. In both stages, `classification_loss` is the same weighted softprob cross-entropy (`Σ w·(-log p_true)`), `regularization_loss` is reconstructed from the trained trees using the same `gamma` / `reg_lambda` / `reg_alpha` definition, and `total_loss` adds that regularization term on top of the classification and decorrelation pieces. The decorrelation term is scaled once from the saved stage-1 best model using `(cls_loss_stage1 + reg_loss_stage1) / max(raw_decor_loss_stage1, ε)`, so the stage-2 decorrelation term stays on the same overall scale as the stage-1 endpoint. Early stopping is stage-local and uses test `total_loss`. The saved reference files keep the pre-lumi physics test weights, and downstream comparison multiplies them by the configured `lumi` before checking the reconstructed physical weights.
...
- `selections/BDT/config.json`: BDT inputs, class groups, training settings, output directories, top-level `decor_loss_mode`, `decor_lambda`, `decor_n_bins`, `decor_n_thresholds`, `decor_score_tau`, `decor_bin_tau_scale`, and per-tree hyperparameters (`n_estimators`, `n_estimators_decorr`, `max_depth`, `learning_rate`, `learning_rate_decorr`, `gamma`, `reg_lambda`, `reg_alpha`, `min_child_weight`, `early_stopping_rounds`), `decorrelate`, and `event_reweight_branches` (per-event raw-weight branches multiplied before sample normalisation; default `["weight_pu"]`).
