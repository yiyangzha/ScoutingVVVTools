...
3. `mode=6`: shuffle each selected MC sample across its ROOT chunks while preserving chunk sizes and filenames.
...
- `mode=6`: compile and run `selections/mix/mix.C`.
  ...
  Output: shuffled ROOT files under the configured `output_root/{signal|bkg}/`, preserving the original chunk filenames and per-chunk entry counts while randomising the global per-tree entry order for each selected MC sample, plus `selections/mix/log.txt`.
...
- `selections/mix/config.json`: mix-step tree selection, input/output ROOT roots and patterns, sample config, threading, and deterministic `random_state`.
...
- `mode=6` can rewrite those converted MC trees into a shuffled mirror with the same chunk layout, which the checked-in BDT config uses by default.
