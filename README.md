...
Mode 11 resolves DAS datasets before submission. If pixi shadows a working CMS `dasgoclient`, set `SCALE_FACTOR_DASGOCLIENT=/path/to/dasgoclient`; otherwise the controller prefers a `dasgoclient` outside `CONDA_PREFIX`. DAS queries run with pixi/conda variables removed while preserving the external PATH/proxy shell environment.
