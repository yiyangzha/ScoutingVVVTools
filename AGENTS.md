...
- Ntuple production keeps `qcd-mg` for the standalone QCD mistag-WP scan and the formal template/fit inputs. Generated Scouting topwsf cards map it to an independent `qcd` process shown separately in validation, prefit, and postfit plots. Like `other`, QCD has both a pass/fail `SF_qcd` parameter and its own `qcd_xsec` `lnU 5.0` normalization nuisance; its per-sample nominal cross sections come from `src/sample.json`.
