...
- Data samples: non-official ScoutingNano 2024G-I, one sample per era named like `2024G`, requiring `DST_PFScouting_SingleMuon`.
...
`systematics/scale_factor/check_empty_mc_inputs.py` is a read-only lxplus diagnostic for the selected scale-factor MC inputs. It resolves the current `ntuple_config.json` samples with DAS, records every source file's `Events` entry count plus `Runs/genEventCount` (when present) and `Runs/genEventSumw`, and reports the MC normalization inflation predicted if zero-entry inputs were dropped. Its Events/Runs-count ratio checks the unweighted source event accounting; `genEventSumw` remains the actual MC normalization denominator. With `--xsec-weights`, it also compares those sums to the denominator actually used by topwsf step 1. It does not run conversion or alter ntuples. Mode 12 now independently resolves every selected original MC ScoutingNanoAOD dataset through DAS/XRootD, records its complete `Runs/genEventSumw`, zero-Events-file contribution, and LHE weighted sums under `generated/normalization/{production_version}/`, embeds the aggregate in each generated topwsf card, and uses it instead of the merged-ntuple `Runs` denominator. It additionally refuses a run when the existing merged-ntuple denominator is neither the complete source sum nor the expected sum with only zero-Events source files omitted.
