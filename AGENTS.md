...
For `/.../USER` dataset resolution, scale-factor `nano.cpp` first tries the legacy DAS `instance=prod/phys03` query and then falls back to `system=rucio`, `system=dbs3`, and plain DAS queries. The controller sets `SCALE_FACTOR_DASGOCLIENT` to a `dasgoclient` outside `CONDA_PREFIX` when possible; users may override it explicitly if pixi shadows the working CMS client. DAS queries run with pixi/conda identity variables removed and active `CONDA_PREFIX` paths stripped from path-like variables, while preserving non-conda CMS/IHEP runtime paths and proxy environment because the IHEP CVMFS `dasgoclient` behaves differently inside the pixi runtime environment.
