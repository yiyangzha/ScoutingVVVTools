...
- **[selections/mix/config.json](selections/mix/config.json)** — controls the optional mix step: selected trees, input/output ROOT roots and patterns, sample config, thread count (used for the OpenMP input-chunk scan), and deterministic `random_state` used for per-tree block shuffling while preserving chunk layout.
