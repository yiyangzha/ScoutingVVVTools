...
`selections/mix/mix.C` can read that same layout and write a shuffled mirror under a different `output_root`. For each selected MC sample and requested tree, it concatenates all chunk files through a `TChain`, applies a deterministic global permutation seeded by `random_state` (offset per tree), and writes the entries back into output files that preserve the original chunk filenames and per-chunk entry counts. This keeps downstream path conventions unchanged while randomising the sequential entry order used by fixed train/test splits.
...
- **[selections/mix/config.json](selections/mix/config.json)** — controls the optional mix step: selected trees, input/output ROOT roots and patterns, sample config, thread count, and deterministic `random_state` used for per-tree shuffling while preserving chunk layout.
