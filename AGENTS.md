...
The scale-factor ntuple config uses `ntuple.build_jobs` for the local `nano.cpp` build; `"auto"` uses the visible CPU count. Condor ntuple jobs still request one CPU, use `AcctGroup="cms"` and IHEP `HepJob_WallTime="mid"` in the generated JDL, and worker builds use all CPUs visible inside the slot unless `BUILD_JOBS` is set.
