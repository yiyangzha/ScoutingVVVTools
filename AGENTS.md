...
- AK8 W jets only.
- The active default configuration selects only non-official 2024C-I data and 2024 MC; no 2025 data or MC is selected. `ntuple.official_configs` remains dormant support for a future explicit `_official` selection. Official cards bind the legacy `ScoutingPFJetRecluster` AK4 schema, while all active data and MC use `ScoutingPFJetRecluster2`; do not substitute the nonexistent `ScoutingPFJetRecluster_scoutUParT_probb` branch for an official input.
...
- Default scale-factor configs use `production_version: "v1"`. This value is appended after `nano_version` in local merged-ntuple, Tier pieces, and annual fit-input directories so separate productions cannot merge stale pieces while job-directory and submission behavior remain unchanged.
- Data samples: non-official ScoutingNano 2024C-I, one sample per era named like `2024C`, requiring `DST_PFScouting_SingleMuon`.
...
Remote ScoutingNano inputs may be DAS datasets, `/store/...` files, `root://...` files, local directories, local ROOT files, or text/list/YAML file lists. The scale-factor default config stages remote ROOT files with `xrdcp` into worker scratch when available before processing on batch workers because direct ROOT streaming from IHEP workers to the CMS global redirector can time out; do not otherwise change remote staging behavior unless requested. POSIX-mounted `/eos/...` paths, such as lxplus `/eos/home-*` workspaces, are allowed as local paths; Tier output paths must use `root://...`. When `ntuple.use_tier_storage` is true, step-1 Condor/HepJob workers copy each per-job merged piece to the configured Tier path, and `nano_merge` can later read those Tier pieces with `--pieces-dir` while writing merged ntuples locally. With `split_by_era` enabled, step 1 creates one data job, Tier pieces directory, and local merged directory per data era, plus exactly one MC job per configured year under `{year}_mc_{nano_version}_{production_version}`; MC is not repeated for each data era. Before step 2, mode 12 creates the local annual `{year}_{nano_version}_{production_version}` fit-input directory as a non-copying per-file symbolic-link union of every era's merged data and that year's merged MC. It then generates one topwsf card and fit per year and tagger, with all annual data and the one annual MC set. Step-2 topwsf input remains local through `calibration.input_sample_base`.
