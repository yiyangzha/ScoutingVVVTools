...
`signal_region.py` reads all parameters from [selections/signal_region/config.json](selections/signal_region/config.json). It loads the saved model, branch, and selection configs from the `bdt_root` directory written by `train.py`, reloads the exact test split defined in `test_ranges.json`, applies physics-normalised weights (`lumi × σ × N_tree / N_raw`), converts the stored reference weights to the same scale by multiplying them with the local `lumi`, validates that its reconstructed test-split prediction matches `test_reference_signal_region.npz` within the stored tolerances, and then builds `N = n_signal_regions` non-overlapping general high-dimensional rectangles in the `D = NUM_CLASSES − 1` dimensional BDT score space (each axis has independent low/high). It does this sequentially: scan once to find the best remaining rectangle, add it to the forbidden set, then rerun the same scan while rejecting any candidate that overlaps previously selected SRs until N regions are selected. Each scan builds dense per-axis boundaries from total/signal/background weighted quantiles plus tail-heavy quantiles, runs deterministic beam coordinate search, and refines the strongest candidates with nearby real event score thresholds. The per-scan final pick uses bounded beam selection over the top `final_selection_candidates`, with the OpenMP helper [openmp_region_select.cpp](selections/signal_region/openmp_region_select.cpp) when `max_threads > 1` and an OpenMP compiler/libomp setup is available; `max_threads == 1` disables the OpenMP selector. The build tries common Homebrew libomp flags and generic `-fopenmp`, then falls back to the same bounded Python selector if unavailable. Plots are saved as `sr_score_*.pdf`, `sr_regions_2d_no_regions.pdf`, and `sr_regions_2d.pdf` in the configured `output_dir` (defaulting to `bdt_root` for backward compatibility). The 2D region views project the full multiclass probability vector onto the vertices of a regular `NUM_CLASSES`-gon; the no-region version shows class-colored transparent test points and class-labeled vertices, while the region version overlays projected high-dimensional SR polygons instead of rectangles. The selected high-dimensional region definitions are written to `signal_region.csv` in the same directory.
...
For each scenario combine is run twice: once using the MC-true group/sample blocks directly from the ROOT file, once replacing all QCD classes (matched by `class_groups` names containing `qcd` case-insensitively) with the single merged `qcd_predict` block while keeping the other backgrounds on their MC-true group/sample blocks. Every channel is validated before running: all configured groups must exist, every configured signal sample must exist, and the code refuses to regularize, clamp, or synthesize missing/zero-rate inputs. If any required signal sample is absent, any covariance block is invalid, or any shape nuisance derived from the covariance would require modifying the input yields, the program aborts instead of patching the model. The ROOT covariance bundles are inserted and assigned without going through incompatible placeholder matrix sizes, so loading `samples/*`, `groups/*`, and `qcd_predict` does not emit ROOT `matrices not compatible` errors. When `rescale_shape_modes_to_positive` is enabled in [combine/config.json](combine/config.json) (default `true`), any background process whose nominal yields and covariance are identically zero across all SRs is dropped before writing the datacard, and any covariance eigenmode whose raw `±1σ` templates would make a varying bin negative or a whole-template norm non-positive is shrunk to the largest safe step below that boundary with the datacard `shape` coefficient scaled by `1/a`; zero-valued bins are then allowed as long as the varied templates stay non-negative and keep strictly positive total norms, and every such drop/rescale is logged as a warning in `combine/log.txt`. If a `sample` scenario's signal process is identically zero in some channels, those channels are skipped with a warning; if the signal sample is identically zero in every channel, the wrapper logs a warning, writes `0` into the significance CSV field and `inf` into the limit CSV fields, and continues with the remaining scenarios. If `AsymptoticLimits` returns successfully but its ROOT output still lacks the expected quantiles, the wrapper logs a warning, writes `0` into the corresponding significance CSV field and `inf` into the limit CSV fields, and continues with the remaining scenarios. With the option disabled, the stricter zero-bin rejection remains fatal and those zero background processes are kept.

To preserve the full per-process SR-to-SR covariance inside combine, each process's `covariance_total` is eigen-decomposed and every retained eigenmode (eigenvalue `> eigen_rel_cutoff × max(diag(cov))`) becomes one Gaussian shape nuisance. By default the templates start from `Up_i = rate_i + √λ_k · V_{i,k}` and `Down_i = rate_i − √λ_k · V_{i,k}`; if that would make any bin or whole-template norm non-positive and `rescale_shape_modes_to_positive=true`, the wrapper instead writes `Up_i = rate_i + a·√λ_k · V_{i,k}` and `Down_i = rate_i − a·√λ_k · V_{i,k}` with the largest safe `a < 1`, then compensates in the datacard with a `shape` coefficient of `1/a`. Per-channel datacards and shape ROOT files are written under `output_dir/work/<qcd_mode>_<scope>_<name>/` and stitched together with `combineCards.py`. `combine` is then invoked as `-M Significance -t -1 --expectSignal 1` and `-M AsymptoticLimits -t -1 --run expected`, and the resulting `higgsCombine*.root` files are parsed to fill four CSVs in `output_dir`: `significance.csv`, `limits.csv`, `significance_abcd_mc.csv`, and `limits_abcd_mc.csv`. The MC-true and ABCD CSVs share the same row order (scope, name). When `keep_work: true` (default) the generated datacards, shape ROOT files, and combine outputs are left in place for inspection; with `keep_work: false` the work directory is removed after the final CSV is written.
...
- **[selections/signal_region/config.json](selections/signal_region/config.json)** — controls signal_region.py: `lumi` (fb⁻¹), `N` / `n_signal_regions` (number of signal regions), `bdt_root` (trained tree output directory to read from, relative to `signal_region/`), `output_dir` (directory for saved PDFs and `signal_region.csv`, relative to `signal_region/`, defaulting to `bdt_root`), `min_bkg_weight` (minimum background weight per region), `max_edge_candidates_per_axis` (maximum distinct boundary values per score axis, including `0` and `1`), `beam_width`, `top_intervals_per_axis`, `coordinate_rounds`, `local_refine_rounds`, `local_refine_neighbor_edges`, `local_refine_top_candidates`, `candidate_pool_limit`, `final_selection_candidates`, `selection_beam_width`, `max_threads`, `progress_every_seconds`, and `seed_quantiles`.
...
- Combined objective: `Z_comb = sqrt(Σ Z_i²)` over the N selected non-overlapping regions; regions are selected sequentially, with each round rerunning the scan on candidates that do not overlap the already selected SRs and choosing the best remaining region.
...
- Beam coordinate search: seeds include the full box plus high- and low-side single-axis seeds from `seed_quantiles`; after the first SR, extra seed slabs and pairwise segment seeds are generated from the already selected SR boundaries so the fresh scan can start in regions that are geometrically disjoint from previous SRs. For up to `coordinate_rounds`, every beam state updates every axis by histogramming events that pass the other axes, sweeping all `(a < b)` intervals in `O(K²)`, and keeping the best `top_intervals_per_axis` exact intervals for that axis. The beam keeps the strongest states while preserving some non-overlapping diversity, and every valid visited rectangle is added to the candidate pool.
- Local event-threshold refinement: the best `local_refine_top_candidates` regions are refined for `local_refine_rounds` passes. Around each current low/high boundary, the scan injects nearby real event score values from the cached per-axis score values (`local_refine_neighbor_edges`) and again performs exact one-axis interval sweeps, adding each valid refined rectangle to the pool.
- Candidate pool: every region is deduped by rounded `(low, high)`, gated by `B ≥ min_bkg_weight` and `S > 0`, and bounded by `candidate_pool_limit`; if the limit is exceeded the pool is trimmed by region significance and a warning is logged.
- Final per-scan pick: the previous unbounded branch-and-bound DFS is intentionally not used for production because it can run for many hours on O(10k) candidates. Each sequential scan instead passes the top `final_selection_candidates` regions through bounded beam selection with width `selection_beam_width` and `target_regions=1`. When `max_threads > 1`, `signal_region.py` compiles and loads `openmp_region_select.cpp` into `output_dir/.openmp/` and uses at most `max_threads` OpenMP threads for that selection, subject to available processors. When `max_threads == 1`, it uses the bounded Python selector without trying OpenMP. The build tries common Homebrew libomp flags and generic `-fopenmp`; if build/load fails, it logs a warning and uses the same bounded beam selection in Python. Progress logs report sequential SR scans, beam rounds, local refinement progress, candidate-pool size, final selector type, and final `Z_comb`.
- Signal class: any class where all member samples have `is_signal=true`. Background: all other classes.
- Output files: `sr_score_*.pdf`, `sr_regions_2d_no_regions.pdf`, `sr_regions_2d.pdf`, and `signal_region.csv` all go to `output_dir`. The CSV stores one row per selected signal region and expands the high-dimensional bin definition into per-axis `{axis_name}_low` / `{axis_name}_high` columns.
