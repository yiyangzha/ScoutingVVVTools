...
For every channel it loads the stored yield/covariance bundles from `samples/{sample}/{yield,covariance_total}`, `groups/{group}/{yield,covariance_total}`, and `qcd_predict/{yield,covariance_total}`. It then iterates over scenarios:
