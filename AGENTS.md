...
The scale-factor ntuple config uses `ntuple.build_jobs` for the local `nano.cpp` build; `"auto"` uses the visible CPU count. Generated ntuple job directories include `submit.sh`; on IHEP `lxlogin*` hosts it stages the current X509 proxy as `x509up_proxy` and submits with `hep_sub ./process.sh -g cms -wt mid ...`, on CERN `lxplus*` hosts it stages the current proxy as `x509up_proxy` and submits `submit_lxplus.jdl` with that staged proxy in `transfer_input_files`, CERN-style `+JobFlavour = "tomorrow"` and `MY.WantOS = "el9"` instead of IHEP accounting/walltime ClassAds, and loads `lxbatch/eossubmit` first when the job directory is under `/eos/...`, and other hosts fall back to `condor_submit submit.jdl` with `use_x509userproxy = true`. The IHEP `hep_sub` path passes `download_remote_inputs` as the eighth worker argument so remote input staging works there too. Condor ntuple jobs still request one CPU, and worker builds use all CPUs visible inside the slot unless `BUILD_JOBS` is set. The generated worker script prints proxy, XRootD, host, and input diagnostics, sets XRootD to IPv4 by default, tries multiple CMS XRootD redirectors for remote input staging via `SCALE_FACTOR_XRD_REDIRECTORS`, and guards shared tarball extraction and `nano.cpp` build steps with `flock` because all ProcId jobs in one generated work directory reuse the same tarball-hash-specific repository and build directory.
