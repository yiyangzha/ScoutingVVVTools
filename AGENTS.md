...
For `/.../USER` dataset resolution, scale-factor `nano.cpp` first tries the legacy DAS `instance=prod/phys03` query and then falls back to `system=rucio`, `system=dbs3`, and plain DAS queries. The controller sets `SCALE_FACTOR_DASGOCLIENT` to a `dasgoclient` outside `CONDA_PREFIX` when possible; users may override it explicitly if pixi shadows the working CMS client. DAS queries run with pixi/conda variables removed while preserving the external PATH/proxy shell environment because the IHEP CVMFS `dasgoclient` behaves differently inside the pixi runtime environment.
