...
- Final selection: a branch-and-bound DFS picks the exact best set of N non-overlapping regions from the candidate pool maximising `Σ Z_i²` whenever such a set exists. If no exact N-region set exists in the candidate pool, a warning is logged and the best smaller non-overlapping subset is returned. Progress logs report beam rounds, local refinement progress, candidate-pool size, DFS visited/pruned counts, and final `Z_comb`.
