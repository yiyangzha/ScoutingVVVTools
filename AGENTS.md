...
- Candidate pool: every region is deduped by rounded `(low, high)`, gated by `B ≥ min_bkg_weight`, `S > min_signal_weight`, and optional event-count minima, and bounded by `candidate_pool_limit`; if the limit is exceeded the pool is trimmed by region significance and a warning is logged. Before global selection, exact event-mask deduplication is applied to the full candidate pool first, then `global_selection_candidates` limits the resulting unique candidates. This avoids losing lower-ranked but complementary unique candidates when many high-ranked geometric variants select the same events.
