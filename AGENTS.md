...
- For each data sample in `data_samples`, loads every event with `weight = 1` per event. Data samples do not read the reweight branches. If the BDT config points MC at `{sample_group}_mixed`, data loading first tries the configured pattern and then falls back from `{sample_group}_mixed` to `{sample_group}` so converted data can remain under the original `data/` directory.
