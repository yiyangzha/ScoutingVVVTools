...
`signal_region.py` reads all parameters from [selections/signal_region/config.json](selections/signal_region/config.json). It loads the saved model, branch, and selection configs from the `bdt_root` directory written by `train.py`, reloads the exact test split defined in `test_ranges.json`, applies physics-normalised weights (`lumi × σ × N_tree / N_raw`), converts the stored reference weights to the same scale by multiplying them with the local `lumi`, validates that its reconstructed test-split prediction matches `test_reference_signal_region.npz` within the stored tolerances, and then builds `N = n_signal_regions` non-overlapping general high-dimensional rectangles in the `D = NUM_CLASSES − 1` dimensional BDT score space (each axis has independent low/high). It first builds one shared candidate pool using dense per-axis boundaries from total/signal/background weighted quantiles plus tail-heavy quantiles, deterministic beam coordinate search, and optional nearby real event-threshold refinement. Candidates are filtered by weighted signal/background minima, optional event-count minima, geometric validity, and optional duplicate event-mask removal. The final selection is global rather than sequential: it chooses K mutually non-overlapping candidates that maximize the existing combined objective `sqrt(sum Z_i^2)`, using the same per-bin Asimov significance calculation already used by the script. With `max_threads > 1` and a working OpenMP compiler/libomp setup, [openmp_region_select.cpp](selections/signal_region/openmp_region_select.cpp) uses a beam incumbent followed by branch-and-bound over the candidate list; if the configured time/node limits stop the search before exhaustion, the script reports the best found Z and a conservative upper-bound certificate. With `max_threads == 1` or if the helper cannot build, it falls back to the Python branch-and-bound implementation. Plots are saved as `sr_score_*.pdf`, `scores_no_regions.pdf`, and `scores.pdf` in the configured `output_dir` (defaulting to `bdt_root` for backward compatibility). The 2D region views project the full multiclass probability vector onto the vertices of a regular `NUM_CLASSES`-gon; the no-region version shows class-colored transparent test points and class-labeled vertices, while the region version overlays projected high-dimensional SR polygons instead of rectangles. The selected high-dimensional region definitions are written to `signal_region.csv` in the same directory.
...
- **[selections/signal_region/config.json](selections/signal_region/config.json)** — controls signal_region.py: `lumi` (fb⁻¹), `N` / `n_signal_regions` (number of signal regions), `bdt_root` (trained tree output directory to read from, relative to `signal_region/`), `output_dir` (directory for saved PDFs and `signal_region.csv`, relative to `signal_region/`, defaulting to `bdt_root`), `min_bkg_weight` / `min_signal_weight` (minimum weighted yields per region), optional `min_signal_entries` / `min_bkg_entries` event-count minima, `max_edge_candidates_per_axis` (maximum distinct boundary values per score axis, including `0` and `1`), candidate-generation controls `beam_width`, `top_intervals_per_axis`, `coordinate_rounds`, `local_refine_rounds`, `local_refine_neighbor_edges`, `local_refine_top_candidates`, and `candidate_pool_limit`, global-selection controls `global_selection_candidates`, `global_beam_width`, `branch_bound_max_nodes` (`0` for no node cap), `branch_bound_time_limit_seconds` (`0` for no time cap), `deduplicate_event_masks`, `require_exact_n_regions`, `max_threads`, `progress_every_seconds`, and `seed_quantiles`.
...
- Candidate pool: every region is deduped by rounded `(low, high)`, gated by `B ≥ min_bkg_weight`, `S > min_signal_weight`, and optional event-count minima, and bounded by `candidate_pool_limit`; if the limit is exceeded the pool is trimmed by region significance and a warning is logged. Before global selection, the top `global_selection_candidates` candidates can also be deduped by exact event mask.
