...
- Within each class, the ratio of `target_total` between samples is proportional to `xsection * tree_entries_total / raw_entries`. Across classes, the class-level totals are then rebalanced to be equal by the existing class-balancing step.
