...
- In the custom-objective decorrelation modes (`smooth_cvm` and `cvm`), `classification_loss` is the native weighted-sum softprob loss built from the class-balanced unit-scale training weights, and its grad/hess follow the same native XGBoost per-row formulas. The decorrelation term is multiplied by one fixed train-split scale calibrated from a very short classification-only pilot fit using the ratio of native classification vs unit-scale decorrelation gradient/Hessian norms, so the fixed scale reflects the early training regime instead of a later pure-classification solution; `smooth_cvm` uses that same scaled definition in the differentiable objective, while `cvm` uses the same scaled loss/output together with its existing legacy surrogate gradient. The tree regularization stays entirely inside XGBoost's native `Ω(f)` handling, and `train.py` reconstructs that cumulative regularization from the trained trees each round so the logged `total_loss` and early stopping both use `classification_loss + decorrelation_loss + regularization_loss`. `train.py` still prints train/test `classification_loss`, `decorrelation_loss`, and `total_loss` each round, with the regularization contribution folded into `total_loss`.
