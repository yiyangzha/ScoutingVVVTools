...
- Combined objective: `Z_comb = sqrt(Σ Z_i²)` over the N selected non-overlapping regions; the search picks the N regions that maximise `Σ Z_i²` jointly, not greedily on individual `Z`.
...
- Final selection: the previous unbounded branch-and-bound DFS is intentionally not used for production because it can run for many hours on O(10k) candidates. Instead, the top `final_selection_candidates` regions are passed through bounded beam set-packing with width `selection_beam_width`; this is approximate but keeps runtime bounded. When `max_threads > 1`, `signal_region.py` compiles and loads `openmp_region_select.cpp` into `output_dir/.openmp/` and uses at most `max_threads` OpenMP threads for the set-packing expansion, subject to available processors. When `max_threads == 1`, it uses the bounded Python selector without trying OpenMP. The build tries common Homebrew libomp flags and generic `-fopenmp`; if build/load fails, it logs a warning and uses the same bounded beam selection in Python. Progress logs report beam rounds, local refinement progress, candidate-pool size, final selector type, and final `Z_comb`.
