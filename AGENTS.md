...
BDT outputs are written under the per-tree `output_root` configured in `selections/BDT/config.json` (for example `selections/BDT/fat2/`). Models are saved there together with concise PDF summaries such as `importance.pdf`, `loss_classification.pdf`, `loss_decorrelation.pdf`, `loss_total.pdf` (all three loss plots use a y-axis that starts at 0), `feature_corr.pdf`, `decor_corr_train.pdf`, one `roc_{sig_name}.pdf` per signal class with all background classes overlaid in distinct colors, and per-signal-vs-background `score_*` plots. Each tree output directory also stores copies of `config.json`, `branch.json`, `selection.json`, the fixed per-sample test split definition `test_ranges.json`, and the saved test-set prediction references `test_reference_signal_region.npz` / `test_reference_qcd_est.npz`. During training, `train.py` prints six loss fields per boosting round (train/test classification, decorrelation, and total). In the custom-objective decorrelation modes (`smooth_cvm` and `cvm`), the classification term uses native weighted-sum XGBoost softprob scale on class-balanced unit-scale training weights (`Σ w·(-log p_true)` with per-row native softprob grad/Hess), the tree regularization term is kept entirely in XGBoost's native `Ω(f)` machinery (`gamma`, `reg_lambda`, `reg_alpha`), and the logged `total_loss` adds that native regularization term back on top of the recorded classification and decorrelation losses. The decorrelation term is multiplied by one fixed train-split scale calibrated once from a longer classification-only pilot fit of `max(20, n_estimators // 4)` rounds so the pilot reaches a regime where scores have separated and `raw_decor_loss` is no longer near-zero; the scale is then set to `(cls_loss_pilot + reg_loss_pilot) / max(raw_decor_loss_pilot, ε)` so that `decor_lambda=1` makes the logged `decor_loss` comparable in magnitude to `cls_loss + reg_loss` at that pilot point. To keep the Gauss-Newton surrogate from underestimating curvature and overshooting near convergence, the per-event decor Hessian is floored at `c·|grad|` before being added to the classification Hess; `c` is set from the same pilot as `median(hess_cls) / median(|grad_cls|)` by default (top-level `decor_hess_floor_c="pilot"`), but can be overridden to any non-negative number to keep `c` fixed. The saved reference files keep the pre-lumi physics test weights (`σ × N_tree / N_raw` with the same per-event raw-w shaping as the downstream scripts), and downstream comparison multiplies those stored weights by the local analysis `lumi` before checking them against the reconstructed physical weights.
...
- **[selections/BDT/config.json](selections/BDT/config.json)** — controls BDT training inputs and outputs: `submit_trees`, `class_groups`, `output_root`, `model_pattern`, `entries_per_sample`, `train_fraction`, top-level `decor_loss_mode` (`smooth_cvm` by default, or legacy `cvm`), top-level `decor_hess_floor_c` (`"pilot"` by default, or any non-negative number to fix the decor Hessian floor coefficient), and per-tree hyperparameters (`n_estimators`, `max_depth`, `learning_rate`, `gamma`, `reg_lambda`, `reg_alpha`, `min_child_weight`, `early_stopping_rounds` with default 10) plus `decorrelate` and `event_reweight_branches` (list of per-event reweight branches multiplied together before sample normalisation; default `["weight_pu"]`).
...
- Within each class, the ratio of `target_total` between samples is proportional to `xsection * tree_entries_total / raw_entries`. Across classes, the class-level totals are then rebalanced to be equal by the existing class-balancing step, with each class total scaled to `N_loaded / N_classes` so the overall training-weight scale stays close to the number of loaded events instead of the old large fixed constant.
...
- If `decorrelate` is non-empty and `decor_lambda > 0`, `train.py` uses the top-level `decor_loss_mode` to choose the decorrelation term. `smooth_cvm` is the default differentiable mode: it matches the old CvM objective conceptually by comparing class-wise score-efficiency curves across decorrelation-variable bins, but replaces the hard bin/CDF steps with smooth sigmoid surrogates so the recorded decorrelation loss, total loss, and custom training gradient all refer to the same smooth objective. `cvm` keeps the original hard-bin CvM-style loss and its legacy surrogate gradient for backward-compatible studies.
- In the custom-objective decorrelation modes (`smooth_cvm` and `cvm`), `classification_loss` is the native weighted-sum softprob loss built from the class-balanced unit-scale training weights (`Σ w·(-log p_true)`), and its grad/hess follow the same native XGBoost per-row softprob formulas. The decorrelation term is multiplied by one fixed train-split scale calibrated from a longer classification-only pilot of `max(20, n_estimators // 4)` rounds, where scores have actually separated and `raw_decor_loss` reflects a realistic (non-near-zero) magnitude; the scale is set to `(cls_loss_pilot + reg_loss_pilot) / max(raw_decor_loss_pilot, ε)` so `decor_lambda=1` keeps the logged `decor_loss` comparable in magnitude to `cls_loss + reg_loss` at the pilot point, and it is held fixed for the remainder of training. `smooth_cvm` uses that same scaled definition in the differentiable objective; `cvm` uses the same scaled loss/output together with its existing legacy surrogate gradient. The per-event decor Hessian is floored at `c·|grad|` before being added to the classification Hessian to stop XGBoost's Newton step from overshooting when the Gauss-Newton surrogate underestimates curvature; `c` is taken from the same pilot as `median(hess_cls) / median(|grad_cls|)` when top-level `decor_hess_floor_c="pilot"`, or from the configured number otherwise. The tree regularization stays entirely inside XGBoost's native `Ω(f)` handling, and `train.py` reconstructs that cumulative regularization from the trained trees each round so the logged `total_loss` and early stopping both use `classification_loss + decorrelation_loss + regularization_loss`. `train.py` still prints train/test `classification_loss`, `decorrelation_loss`, and `total_loss` each round, with the regularization contribution folded into `total_loss`. Early stopping uses the test `total_loss` and stops when it has not improved for `early_stopping_rounds` consecutive rounds (per-tree config, default 10).
- For decorrelated training, the saved JSON model is sliced to the best early-stopping round before being written. Downstream scripts load JSON models through `xgb.Booster`, take raw margins with `output_margin=True`, and apply the multiclass softmax explicitly before using the BDT scores.
...
- ROC and score-distribution plots adapt to the configured classes. When both signal-like and background-like classes exist, one `roc_{sig_name}.pdf` is written per signal class, overlaying every background class in a distinct color (test curves solid, train curves dashed), and one `score_{sig_name}_vs_{bkg_name}.pdf` is written per signal-background pair. When there is no signal/background split, the code falls back to one ROC PDF per class (treating it as the "signal" against all other classes) and all-pair score distributions.
