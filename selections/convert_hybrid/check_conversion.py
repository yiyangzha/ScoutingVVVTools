...
def main():
    ...
    for r in broken:
        ...
        if action == "remerge":
            ...
            lines.append(f"python3 run.py 0 {ctx['config']} {name} --slurm")
            lines.append("")
    ...
    if args.submit:
        ...
        for r in broken:
            ...
            if action == "remerge":
                ...
                print(f"  -> {name}: reprocess (run.py 0 {ctx['config']} {name} --slurm)")
                subprocess.run(["python3", "run.py", "0", ctx["config"], name, "--slurm"],
