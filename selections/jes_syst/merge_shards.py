#!/usr/bin/env python3
"""Merge per-sample jes_syst.py SLURM shard outputs into the final
jes_syst_yields.json/.csv, matching main()'s own output exactly (the
...
def main():
    ...
    for sample in manifest:
        path = os.path.join(SHARDS_DIR, sample, "jes_syst_yields.json")
    ...
    json_path = os.path.join(OUT_DIR, "jes_syst_yields.json")
    json.dump(nested, open(json_path, "w"), indent=2)
    print(f"Wrote {json_path}  ({len(nested)} samples)")

    csv_path = os.path.join(OUT_DIR, "jes_syst_yields.csv")
        ...
        for sample in sorted(nested):
            for tree in sorted(nested[sample]):
                ...
                writer.writerow([sample, tree, "inclusive",
                                  f"{entry['jes_up']:.6f}", f"{entry['jes_down']:.6f}",
                                  entry["n_events"]])
                for bid, rr in entry.get("regions", {}).items():
                    writer.writerow([sample, tree, bid,
                                      f"{rr['jes_up']:.6f}", f"{rr['jes_down']:.6f}",
                                      rr["n_events"]])
