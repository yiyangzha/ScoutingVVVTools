...
Computes AK4/AK8 jet-energy-scale (JES) shape-systematic yield ratios for
all MC samples, per tree and per signal region.

Unlike pileup_syst.py/theory_syst.py (an alternate weight column in the SAME
converted tree), a JES shift changes jet kinematics -- and therefore every
...
correction only) and two JES-shifted variants (jet_pt_correction.variation
= "jes_up"/"jes_down" in convert_branch.C, written to their own dataset
roots -- see selections/convert/config_jes_{up,down}.json). Each is read
...
pileup_syst.py's context/inference/region-masking code).
  ...
  jes_up   = sum(weight_branch on jes_up tree)   / sum(weight_branch on nominal tree)
  jes_down = sum(weight_branch on jes_down tree) / sum(weight_branch on nominal tree)

Outputs (written to output_dir):
  jes_syst_yields.json  -- {sample: {tree: {n_events, jes_up, jes_down,
                                            regions: {bin_id: {...}}}}}
...
_cfg_path = os.environ.get("JES_SYST_CONFIG_PATH", os.path.join(_SCRIPT_DIR, "config.json"))
...
def _ratio_entry(nominal_slot, up_slot, down_slot):
    ...
    return {
        ...
        "jes_up": (up_slot["sum"] / nominal_slot["sum"]) if up_slot is not None else 1.0,
        "jes_down": (down_slot["sum"] / nominal_slot["sum"]) if down_slot is not None else 1.0,
    }


def main():
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    log("jes_syst.py — JES shape-systematic yield ratios (inclusive + per signal region)")
    ...
    for tree_name in SUBMIT_TREES:
        ...
        for sample_name in MC_SAMPLES:
            ...
            log(f"    inclusive: n={incl['n_events']}"
                f"  jes_up={_pct(incl['jes_up'])}  jes_down={_pct(incl['jes_down'])}")
                ...
                for bid in ctx["bin_ids"]:
                    ...
                    if rr is None or rr["n_events"] < MIN_REGION_EVENTS:
                        ...
                        log(f"    SR{bid}: n={rr['n_events']}"
                            f"  jes_up={_pct(rr['jes_up'])}  jes_down={_pct(rr['jes_down'])}")
                entry["regions"] = region_out

            all_results[(sample_name, tree_name)] = entry

    json_path = os.path.join(OUTPUT_DIR, "jes_syst_yields.json")
    nested = {}
    ...
    csv_path = os.path.join(OUTPUT_DIR, "jes_syst_yields.csv")
    with open(csv_path, "w", newline="", encoding="utf-8") as fh:
        writer = csv.writer(fh)
        writer.writerow(["sample", "tree", "region", "jes_up", "jes_down", "n_events"])
        for (sample, tree) in sorted(all_results.keys()):
            entry = all_results[(sample, tree)]
            writer.writerow([sample, tree, "inclusive",
                             f"{entry['jes_up']:.6f}", f"{entry['jes_down']:.6f}", entry["n_events"]])
            for bid, rr in entry.get("regions", {}).items():
                writer.writerow([sample, tree, bid,
                                 f"{rr['jes_up']:.6f}", f"{rr['jes_down']:.6f}", rr["n_events"]])
